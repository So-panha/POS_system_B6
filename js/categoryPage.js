    ...
    function searchList(){
        ...
        for(let row of tableRows){
            let category = row.firstElementChild.nextElementSibling.textContent.toLocaleLowerCase();
            let texts = search.value;
            if(category.includes(texts.toLocaleLowerCase()) == true || search.length == 0){
                row.style.display = '';
            }else{
